...
test_remove_single_tag() {
    ...
    mock_function "validate_file_path" "return 0"

    local output
    output=$(cmd_remove "/tmp/test_remove" "tag2" 2>&1)

    assert_contains "$output" "Removed tag 'tag2' from /tmp/test_remove" "Should show removal confirmation"

    restore_function "load_tags"
    restore_function "validate_file_path"
}
...
test_remove_non_existent_tag() {
    ...
    assert_contains "$output" "Tag 'nonexistent' not found on file /tmp/test_remove" "Should show error for non-existent tag"
...
test_remove_invalid_tag() {
    ...
    assert_contains "$output" "Invalid tag name 'invalid tag'" "Should reject invalid tag"
